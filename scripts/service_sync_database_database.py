...
# Versão:       1.0
#
# Objetivo:     Serviço sincronização de dados da tabela database_database do MySQL para o MongoDB
#
# ----------------------------------------------------------------------------------------------------------------------
...
CollectionMongoDB = BancoDadosMongoDB.database_database

#
# Colunas da query utilizada no MySQL
#

ColunasQuerySelectMySQL = ('action', 'id_control_update','id','created_time','updated_time','acronym','name','regional_index','network_index','cc_index','created_by_id','updated_by_id','flag_bigdata')

#
# Query que será executada no MySQL
#

QuerySelectMySQL = """select cubdr.action,  cubdr.id_control_update,  if(cubdr.action='D',  cubdr.id_row_object,  t.id), created_time, updated_time, acronym, name, regional_index, network_index, cc_index, created_by_id, updated_by_id,'L' as flag_bigdata from database_database t right join control_updates_bigdata_rows cubdr on t.id = cubdr.id_row_object where (t.id in (select id_row_object from control_updates_bigdata_rows where object = 'database_database' and cubdr.object = 'database_database' and processing_indicator = 'N') or cubdr.action = 'D') and cubdr.processing_indicator = 'N' order by cubdr.instant"""
