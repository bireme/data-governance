...
# Versão:       1.0
#
# Objetivo:     Serviço sincronização de dados da tabela main_descriptor do MySQL para o MongoDB
#
# ----------------------------------------------------------------------------------------------------------------------
...
CollectionMongoDB = BancoDadosMongoDB.main_descriptor

#
# Colunas da query utilizada no MySQL
#

ColunasQuerySelectMySQL = ('action','id_control_update','id','created_time','updated_time','created_by_id','updated_by_id','object_id','content_type_id','text','code','status','primary','flag_bigdata')

#
# Query que será executada no MySQL
#

QuerySelectMySQL = """select cubdr.action, cubdr.id_control_update, if(cubdr.action='D', cubdr.id_row_object, t.id), created_time, updated_time, created_by_id, updated_by_id, object_id, content_type_id, text, code, status, `primary`, 'L' as flag_bigdata from main_descriptor t right join control_updates_bigdata_rows cubdr on t.id = cubdr.id_row_object where (t.id in (select id_row_object from control_updates_bigdata_rows where object = 'main_descriptor' and cubdr.object = 'main_descriptor' and processing_indicator = 'N') or cubdr.action = 'D') and cubdr.processing_indicator = 'N' order by cubdr.instant"""
