...
# Versão:       1.0
#
# Objetivo:     Serviço sincronização de dados da tabela biblioref_referencelocal do MySQL para o MongoDB
#
# ----------------------------------------------------------------------------------------------------------------------
...
CollectionMongoDB = BancoDadosMongoDB.biblioref_referencelocal

#
# Colunas da query utilizada no MySQL
#

ColunasQuerySelectMySQL = ('action','id_control_update','id','call_number','database','inventory_number','internal_note','local_descriptors','cooperative_center_code','source_id','flag_bigdata')

#
# Query que será executada no MySQL
#

QuerySelectMySQL = """select cubdr.action, cubdr.id_control_update, if(cubdr.action='D', cubdr.id_row_object, t.id), call_number, `database`, inventory_number, internal_note, local_descriptors, cooperative_center_code, source_id, 'L' as flag_bigdata from biblioref_referencelocal t right join control_updates_bigdata_rows cubdr on t.id = cubdr.id_row_object where (t.id in (select id_row_object from control_updates_bigdata_rows where object = 'biblioref_referencelocal' and cubdr.object = 'biblioref_referencelocal' and processing_indicator = 'N') or cubdr.action = 'D') and cubdr.processing_indicator = 'N' order by cubdr.instant"""
