...
# Versão:       1.0
#
# Objetivo:     Serviço sincronização de dados da tabela biblioref_referenceanalytic do MySQL para o MongoDB
#
# ----------------------------------------------------------------------------------------------------------------------
...
CollectionMongoDB = BancoDadosMongoDB.biblioref_referenceanalytic

#
# Colunas da query utilizada no MySQL
#

ColunasQuerySelectMySQL = ('action','id_control_update','reference_ptr_id','individual_author','corporate_author','title','english_translated_title','pages','clinical_trial_registry_name','source_id','thesis_dissertation_analytic_leader','doi_number','flag_bigdata')

#
# Query que será executada no MySQL
#

QuerySelectMySQL = """select cubdr.action, cubdr.id_control_update, if(cubdr.action='D', cubdr.id_row_object, t.reference_ptr_id), individual_author, corporate_author,title, english_translated_title, pages, clinical_trial_registry_name, source_id, thesis_dissertation_analytic_leader, doi_number, 'L' as flag_bigdata from biblioref_referenceanalytic t right join control_updates_bigdata_rows cubdr on t.reference_ptr_id = cubdr.id_row_object where (t.reference_ptr_id in (select id_row_object from control_updates_bigdata_rows where object = 'biblioref_referenceanalytic' and cubdr.object = 'biblioref_referenceanalytic' and processing_indicator = 'N') or cubdr.action = 'D') and cubdr.processing_indicator = 'N' order by cubdr.instant"""
