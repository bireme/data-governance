...
# Versão:       1.0
#
# Objetivo:     Serviço sincronização de dados da tabela biblioref_referencecomplement do MySQL para o MongoDB
#
# ----------------------------------------------------------------------------------------------------------------------
...
CollectionMongoDB = BancoDadosMongoDB.biblioref_referencecomplement

#
# Colunas da query utilizada no MySQL
#

ColunasQuerySelectMySQL = ('action','id_control_update','id','conference_sponsoring_institution','conference_name','conference_date','conference_normalized_date','conference_city','project_sponsoring_institution','project_name','source_id','conference_country_id','project_number','flag_bigdata')

#
# Query que será executada no MySQL
#

QuerySelectMySQL = """select cubdr.action, cubdr.id_control_update, if(cubdr.action='D', cubdr.id_row_object, t.id),  conference_sponsoring_institution, conference_name, conference_date, conference_normalized_date, conference_city, project_sponsoring_institution, project_name, source_id, conference_country_id, project_number, 'L' as flag_bigdata from biblioref_referencecomplement t right join control_updates_bigdata_rows cubdr on t.id = cubdr.id_row_object where (t.id in (select id_row_object from control_updates_bigdata_rows where object = 'biblioref_referencecomplement' and cubdr.object = 'biblioref_referencecomplement' and processing_indicator = 'N') or cubdr.action = 'D') and cubdr.processing_indicator = 'N' order by cubdr.instant"""
