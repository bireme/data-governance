...
def create_metric_therapies(country):
    ...
    for result in source_collection.aggregate(pipeline):
        therapy_code = result["_id"]["therapy"]
        therapy = areas_map.get(therapy_code, therapy_code)

        year = result["_id"]["year"]
        logger.info(f"{country}, {therapy}, {year}")
...
with DAG(
    'TMGL_COUNTRIES_02_create_metric_therapies',
    description='TMGL COUNTRIES - Calcula o total de documentos por terapia e ano',
