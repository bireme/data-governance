...
async function therapy_loadDataAndRenderChart() {
    const yearResp = await fetch('{country_iso}_therapy_year.json');
...
def generate_html_therapy(year_from, country, country_iso):
    ...
    documents = list(collection.find({"type": "therapy", "country": country}).sort("year", 1))
    aggregated_data = []
    ...
    output_file = os.path.join(output_dir, f"{country_iso}_therapy_year.json")
