...
async function dimention_loadDataAndRenderChart() {
    const yearResp = await fetch('{country_iso}_dimention_year.json');
...
def generate_html_dimention(year_from, country, country_iso):
    ...
    # Builds year_json
    documents = list(collection.find({"type": "dimention", "country": country}).sort("year", 1))
    aggregated_data = []
    ...
    output_file = os.path.join(output_dir, f"{country_iso}_dimention_year.json")
