...
def generate_html_subject(year_from, country, country_iso):
    ...
    documents = list(collection.find({"type": "subject", "country": country}).sort("year", 1))
    aggregated_data = []
    ...
    output_file = os.path.join(output_dir, f"{country_iso}_subject_year.json")
