...
async function traditional_loadDataAndRenderChart() {
    const yearResp = await fetch('{country_iso}_traditional_year.json');
...
def generate_html_traditional(year_from, country, country_iso):
    ...
    documents = list(collection.find({"type": "traditional", "country": country}).sort("year", 1))
    aggregated_data = []
    ...
    output_file = os.path.join(output_dir, f"{country_iso}_traditional_year.json")
