...
def create_metric_doctype_region():
    logger = logging.getLogger(__name__)
    ...
    for region, countries in regions.items():
        ...
        for result in source_collection.aggregate(pipeline):
            doctype = result["_id"]["doctype"]
            year = result["_id"]["year"]
            logger.info(f"{region}, {doctype}, {year}")
...
with DAG(
    'TMGL_REGION_02_create_metric_doctype',
    ...
    create_metric_doctype_region_task = PythonOperator(
        task_id='create_metric_doctype_region',
        python_callable=create_metric_doctype_region
    )
