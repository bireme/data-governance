...
def create_metric_journals_region():
    logger = logging.getLogger(__name__)
    ...
    for region, countries in regions.items():
        ...
        for result in source_collection.aggregate(pipeline):
            journal = result["_id"]["journal"]
            year = result["_id"]["year"]
            logger.info(f"{region}, {journal}, {year}")
...
with DAG(
    'TMGL_REGION_02_create_metric_journals',
    description='TMGL REGION - Calcula o total de documentos por revista e ano',
    tags=["tmgl", "metrics", "mongodb", "journal", "year"],
    schedule=None,
    catchup=False,
    doc_md=__doc__
) as dag:
    create_metric_journals_task = PythonOperator(
        task_id='create_metric_journals',
        python_callable=create_metric_journals
    )
    create_metric_journals_region_task = PythonOperator(
        task_id='create_metric_journals_region',
        python_callable=create_metric_journals_region
    )
