...
BASE_PIPELINE = [
    {"$match": {"tag_mtc_tema2": {"$exists": True, "$ne": None}}},
    {"$unwind": "$tag_mtc_tema2"},
    {"$addFields": {
        "year": {
            "$toInt": {
                "$ifNull": [
...
def create_metric_complementary():
...
def create_metric_complementary_region():
    logger = logging.getLogger(__name__)
...
with DAG(
    'TMGL_REGION_02_create_metric_complementary',
    description='TMGL REGION - Calcula o total de documentos por complementary medicine e ano',
    tags=["tmgl", "metrics", "mongodb", "complementary", "year"],
    schedule=None,
    catchup=False,
    doc_md=__doc__
) as dag:
    create_metric_complementary_task = PythonOperator(
        task_id='create_metric_complementary',
        python_callable=create_metric_complementary
    )
    create_metric_complementary_region_task = PythonOperator(
        task_id='create_metric_complementary_region',
        python_callable=create_metric_complementary_region
    )
