...
HTML_TEMPLATE = """
const journal_region_year_json = {journal_region_year_json};
const journal_year_json = {journal_year_json};
...
function updateJournalChart() {
    ...
    if (selectedRegion === "Todas") {
        filtered = Object.values(journal_year_json)
            .flat()
            .filter((d) => d.ano >= yearFrom && d.ano <= yearTo);
    } else {
        filtered = journal_region_year_json[selectedRegion].filter(
...
def generate_html_journal():
    ...
    journal_region_year_json = json.dumps(aggregated_data, ensure_ascii=False)
    ...
    journal_year_json = json.dumps(aggregated_data, ensure_ascii=False)

    html_with_data = HTML_TEMPLATE.replace("{journal_region_year_json}", journal_region_year_json)
    html_with_data = html_with_data.replace("{journal_year_json}", journal_year_json)
